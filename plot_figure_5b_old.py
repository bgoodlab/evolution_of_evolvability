...
def run():
    ...
    plt.figure(figsize=(10,4)); plt.xlabel("$U_{d}'$",fontsize=50,labelpad=15); plt.ylabel(r"$ |\overline{s}'|$",fontsize=50,labelpad=15); plt.xscale('log'); plt.yscale('log');
    ...
    plt.plot(Udms,(-1)*s_bars,label="$s_{b}'/s_{b}=1.5$",color='tomato',linewidth=3)
    plt.plot(Udms2,(-1)*s_bars2,label="$s_{b}'/s_{b}=1.2$",color='red',linewidth=3)
